  ...
  ProduceTask::ProduceTask(UnitClass unitClass):
    ...
    void
     ProduceTask::saveGame(XmlNode * rootNode) const {
      ...
      XmlNode *taskNode = rootNode->addChild("Task");
      taskNode->
          addAttribute("taskClass", intToStr(taskClass),
                       mapTagReplacements);
      XmlNode *produceTaskNode = taskNode->addChild("ProduceTask");

//      UnitClass unitClass;
      produceTaskNode->
          addAttribute("unitClass", intToStr(unitClass),
                       mapTagReplacements);
...
//      const ResourceType *resourceType; if (resourceType != NULL)
      {
    ...
    ProduceTask *ProduceTask::loadGame(const XmlNode * rootNode,
                                       Faction * faction) {
...
//      Vec2i pos;
    ...
    BuildTask *BuildTask::loadGame(const XmlNode * rootNode,
                                   Faction * faction) {
     ...
     UpgradeTask::saveGame(XmlNode * rootNode) const {
      ...
      if (upgradeType != NULL)
      {
        ...
        upgradeTaskNode->addAttribute("upgradeType",
                                      upgradeType->getName(),
                                      mapTagReplacements);
    }} UpgradeTask *UpgradeTask::loadGame(const XmlNode * rootNode,
                                          Faction * faction) {
...
// =====================================================
    ...
    void
     Ai::init(AiInterface * aiInterface, int useStartLocation) {
      ...
      if (faction->getAIBehaviorStaticOverideValue
          (aibsvcMinMinWarriorsExpandCpuEasy) != INT_MAX)
      {
        minMinWarriorsExpandCpuEasy =
            faction->getAIBehaviorStaticOverideValue
            (aibsvcMinMinWarriorsExpandCpuEasy);
        //printf("Discovered overriden static value for AI, minMinWarriorsExpandCpuEasy = %d\n",minMinWarriorsExpandCpuEasy);
      }
      if (faction->getAIBehaviorStaticOverideValue
          (aibsvcMinMinWarriorsExpandCpuMega) != INT_MAX)
      {
      ...
      if (faction->getAIBehaviorStaticOverideValue
          (aibsvcMinMinWarriorsExpandCpuUltra) != INT_MAX)
      {
      ...
      if (faction->getAIBehaviorStaticOverideValue
          (aibsvcScoutResourceRange) != INT_MAX)
      {
        scoutResourceRange =
            faction->getAIBehaviorStaticOverideValue
      ...
      if (faction->getAIBehaviorStaticOverideValue
          (aibsvcMinWorkerAttackersHarvesting) != INT_MAX)
      {
      ...
      if (faction->
          getAIBehaviorStaticOverideValue(aibsvcMinBuildSpacing) !=
          INT_MAX)
      {
        minBuildSpacing =
            faction->
...
//      }
    ...
    void
     Ai::update() {
      ...
      {
        const SwitchTeamVote *vote =
            aiInterface->getMyFaction()->getFirstSwitchTeamVote();
        aiInterface->
            getMyFaction()->setCurrentSwitchTeamVoteFactionIndex(vote->
                                                                 factionIndex);
        ...
        SwitchTeamVote *voteResult =
            aiInterface->getMyFaction()->
            getSwitchTeamVote(vote->factionIndex);
        voteResult->voted = true;
        voteResult->allowSwitchTeam = false;

        const GameSettings *settings =
            aiInterface->getWorld()->getGameSettings();

        // If AI player already lost game they cannot vote
        if (aiInterface->
            getWorld()->factionLostGame(aiInterface->getFactionIndex()) ==
            true)
        {
          ...
          if (factionSwitchTeamRequestCountCurrent <= 2)
          {
            ...
            else
            {
              voteResult->allowSwitchTeam =
                  (allowJoinTeam >=
                   (100 - settings->getAiAcceptSwitchTeamPercentChance()));
            }
          }
        }
        ...
        aiInterface->giveCommandSwitchTeamVote(aiInterface->getMyFaction(),
                                               voteResult);
      }
...
// ==================== state requests ====================
    ...
    const ResourceType *Ai::getNeededResource(int unitIndex) {
      ...
      aiInterface->printLog(3, szBuf);
      snprintf(szBuf, 8096,
               "[resource type count %d] Needed resource [%s].",
    ...
    {
      const Unit *enemy =
          aiInterface->getFirstOnSightEnemyUnit(pos, field, radius);
      return (enemy != NULL);
    }
    ...
    bool Ai::findAbleUnit(int *unitIndex, CommandClass ability,
                          bool idleOnly)
    {
      vector < int > units;
      ...
      for (int i = 0; i < aiInterface->getMyUnitCount(); ++i)
      {
            ...
            && unit->getType()->hasCommandClass(ability))
        {
          if (!idleOnly || !unit->anyCommand()
              || unit->getCurrCommand()->getCommandType()->getClass() ==
              ccStop)
          {
            units.push_back(i);
          }
        ...
        if (unit->getType()->isCommandable())
        {
          if (unit->getType()->hasCommandClass(ccHarvest))
          {
                ...
                ccHarvest)
            {
              ...
              const HarvestCommandType *hct = dynamic_cast < const
                  HarvestCommandType * >(command->getCommandType());
              if (hct != NULL)
              {
                const Vec2i unitTargetPos = unit->getTargetPos();
                SurfaceCell *sc =
                    map->getSurfaceCell(Map::toSurfCoords(unitTargetPos));
          ...
          else if (unit->getType()->hasCommandClass(ccProduce))
          {
            ...
            {
              Command *command = unit->getCurrCommand();
              const ProduceCommandType *pct = dynamic_cast < const
          ...
          else if (unit->getType()->hasCommandClass(ccBuild))
          {
            ...
            {
              Command *command = unit->getCurrCommand();
              const BuildCommandType *bct = dynamic_cast < const
...
//}
    ...
    bool Ai::findPosForBuilding(const UnitType * building,
                                const Vec2i & searchPos, Vec2i & outPos)
    {

      for (int currRadius = 0; currRadius < maxBuildRadius; ++currRadius)
      {
        for (int i = searchPos.x - currRadius;
             i < searchPos.x + currRadius; ++i)
        {
          for (int j = searchPos.y - currRadius;
               j < searchPos.y + currRadius; ++j)
          {
            outPos = Vec2i(i, j);
            if (aiInterface->isFreeCells(outPos - Vec2i(minBuildSpacing),
...
// ==================== expansions ====================
    ...
    Vec2i Ai::getRandomHomePosition()
    {
      ...
      return expansionPositions[random.randRange(0,
                                                 (int) expansionPositions.
                                                 size() - 1)];
    }
      ...
      if (megaResourceAttack || ultraResourceAttack)
      {
        ...
        while (possibleTargetFound == false)
        {
              ...
              && map->isInsideSurface(map->toSurfCoords(pos)))
          {
            ...
            Vec2i resPos;
            if (aiInterface->isResourceInRegion(pos, rt, resPos,
                                                scoutResourceRange))
            {
     ...
     Ai::massiveAttack(const Vec2i & pos, Field field, bool ultraAttack) {
      ...
      for (int i = 0; i < unitCount; ++i)
      {
        ...
        const AttackCommandType *act =
            unit->getType()->getFirstAttackCommand(field);
          ...
          if (producerWarriorCount > maxProducerWarriors)
          {
            if (unit->getCommandSize() > 0 &&
                ...
                && (unit->getCurrCommand()->getCommandType()->getClass() ==
                    ccBuild
                    || unit->getCurrCommand()->
                    getCommandType()->getClass() == ccMorph
                    || unit->getCurrCommand()->
                    getCommandType()->getClass() == ccProduce))
            {
                ...
                || aiInterface->getControlType() == ctNetworkCpuMega))
        {
          ...
          std::pair < bool, Unit * >beingAttacked =
              ...
              getUnitUpdater()->unitBeingAttacked(unit);
          if (beingAttacked.first == true)
          {
            Unit *enemy = beingAttacked.second;
            const AttackCommandType *act_forenemy =
                unit->getType()->
                getFirstAttackCommand(enemy->getCurrField());
            ...
            {
              const AttackStoppedCommandType *asct_forenemy =
                  unit->getType()->getFirstAttackStoppedCommand(enemy->
                                                                getCurrField
                                                                ());
...
//                                                                      unit->getId(), enemy->getFullName().c_str(), enemy->getId());
     ...
     Ai::harvest(int unitIndex) {
      ...
      if (rt != NULL)
      {
        const HarvestCommandType *hct =
            ...
            getType()->getFirstHarvestCommand(rt,
                                              aiInterface->
                                              getMyUnit
                                              (unitIndex)->getFaction());
    ...
    bool Ai::getAdjacentUnits(std::map < float, std::map < int,
            ...
            if (pos != unitPos)
            {
                  ...
                  unit->getFactionIndex())
              {
                    ...
                    && adjacentUnit->getPath() != NULL)
                {
                  ...
                  {
                    signalAdjacentUnits[dist][adjacentUnit->getId()] =
                        adjacentUnit;
                  ...
                  else
                  {
                    std::map < int, const Unit *>::iterator
                        iterFind2 =
                        iterFind1->second.find(adjacentUnit->getId());
               ...
               iterMap2 != iterMap->second.end(); ++iterMap2)
          {
                ...
                NULL)
            {
                   ...
                   ++moveAttempt)
              {
                Vec2i
                    pos =
                    Vec2i(random.randRange(-villageRadius * 2,
                                           villageRadius * 2),
...
//    bool randomMinWarriorsReached;
      aiNode->
          addAttribute("randomMinWarriorsReached",
                       intToStr(randomMinWarriorsReached),
                       mapTagReplacements);
//      Tasks tasks;
...
//      Positions expansionPositions;
          for (Positions::const_iterator it = expansionPositions.begin();
               it != expansionPositions.end(); ++it)
      {
        XmlNode *expansionPositionsNode =
            aiNode->addChild("expansionPositions");
    ...
    void
     Ai::loadGame(const XmlNode * rootNode, Faction * faction) {
      ...
      randomMinWarriorsReached =
          aiNode->getAttribute("randomMinWarriorsReached")->
          getIntValue() != 0;
      ...
      for (unsigned int i = 0; i < taskNodeList.size(); ++i)
      {
        ...
        case tcProduce:
          {
            ProduceTask *newTask =
                ProduceTask::loadGame(taskNode, faction);
        ...
        case tcUpgrade:
          {
            UpgradeTask *newTask =
                UpgradeTask::loadGame(taskNode, faction);
            tasks.push_back(newTask);
          }
          break;
        }
      ...
      for (unsigned int i = 0; i < expansionPositionsNodeList.size(); ++i)
      {
        ...
        Vec2i
            pos =
            Vec2i::strToVec2(expansionPositionsNode->
      ...
      //      int minMinWarriorsExpandCpuEasy;
      minMinWarriorsExpandCpuEasy =
          aiNode->getAttribute("minMinWarriorsExpandCpuEasy")->
          getIntValue();
      //      int minMinWarriorsExpandCpuMega;
      minMinWarriorsExpandCpuMega =
          aiNode->getAttribute("minMinWarriorsExpandCpuMega")->
          getIntValue();
      //      int minMinWarriorsExpandCpuUltra;
      minMinWarriorsExpandCpuUltra =
          aiNode->getAttribute("minMinWarriorsExpandCpuUltra")->
          getIntValue();
      //      int minMinWarriorsExpandCpuNormal;
      minMinWarriorsExpandCpuNormal =
          aiNode->
          getAttribute("minMinWarriorsExpandCpuNormal")->getIntValue();
      //      int maxMinWarriors;
      maxMinWarriors =
      ...
      //      int minWorkerAttackersHarvesting;
      minWorkerAttackersHarvesting =
          aiNode->getAttribute("minWorkerAttackersHarvesting")->
          getIntValue();
    }
